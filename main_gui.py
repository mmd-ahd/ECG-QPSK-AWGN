...
# Import functions from individual files within the simulation_functions package
...
class ECGSimApp:
    def __init__(self, master):
        self.master = master
        master.title("ECG Signal Transmission Simulator (Ultra-Modular)")
        ...
        default_record_path = os.path.join("sample_data", "ptb-xl", "records100", "00000", "00001_lr").replace("\\", "/")
        ...
        ttk.Label(control_frame, text="SNR (Eb/N0 in dB):").grid(row=5, column=0, sticky=tk.W, padx=5, pady=2)
    ...
    def run_simulation(self):
        ...
        if not original_bitstream and len(ecg_to_process)>0 : # Check if quantization failed for non-empty signal
        ...
        if current_warnings:
            self.status_label.config(text="Status: Completed with warnings. " + "; ".join(current_warnings), foreground="#FF8C00") # DarkOrange
        else:
            self.status_label.config(text="Status: Simulation Completed Successfully!", foreground="green")

        self.ax1_ecg.clear()
        self.ax1_ecg.plot(time_system, ecg_to_process, label="Original ECG (Post-Resample)", color='blue', alpha=0.7)
        if len(reconstructed_ecg) > 0 and not np.all(np.isnan(reconstructed_ecg)): # Check for NaNs from reconstruction
            ...
            if np.any(valid_reconstructed_indices):
                ...
                else: # Partial plot if lengths mismatch but data exists
        ...
        self.ax1_ecg.set_title("Original (Post-Resample) vs. Reconstructed ECG Signal")
...
def create_dummy_wfdb_files(dummy_record_stem):
    """Creates dummy WFDB .hea and .dat files for basic GUI testing."""
    sample_data_dir = os.path.dirname(dummy_record_stem)
    if not os.path.exists(sample_data_dir):
        try:
            os.makedirs(sample_data_dir, exist_ok=True) # Added exist_ok=True
        except Exception as e:
            print(f"Could not create directory {sample_data_dir}: {e}")
            return False # Indicate failure

    # Create a minimal dummy .hea and .dat file if they don't exist
    # This is a HACK for quick testing without real data.
    # Real WFDB files are complex.
    if not os.path.exists(dummy_record_stem + ".hea") or not os.path.exists(dummy_record_stem + ".dat"):
        try:
            with open(dummy_record_stem + ".hea", "w") as f:
                f.write("00001_lr 2 100 3000 0:0:0 0:0:0\n") # record_name n_channels fs n_samples (30s at 100Hz)
                f.write("00001_lr.dat 212+0 100 0 0 0 I\n")     # filename format+offset gain adc_zero baseline checksum signal_name
                f.write("00001_lr.dat 212+0 100 0 0 0 II\n")    # Using 212 format (12-bit), gain 100
            
            # Create dummy signal data (30 seconds, 100 Hz, 2 channels)
            num_samples = 3000 # 30s * 100Hz
            time_dummy = np.linspace(0, 30, num_samples, endpoint=False)
            # Simulate some ECG-like features, scale to be representable by 12-bit ADC with gain
            # Max ADC value for 12-bit is 2047 (if offset 0). If gain is 100, signal max physical value is ~20mV
            signal_ch1 = (np.sin(2 * np.pi * 1 * time_dummy) + 0.5 * np.sin(2 * np.pi * 10 * time_dummy)) * 5 # mV like values
            signal_ch2 = (np.cos(2 * np.pi * 1 * time_dummy) + 0.3 * np.sin(2 * np.pi * 15 * time_dummy)) * 4 # mV like values

            # Convert to ADC units (physical_value * gain/adc_res (assume adc_res=1 for simplicity with gain))
            # For format 212, it's often (physical_value - baseline) / gain.
            # Or if gain is uV/ADU, then ADU = physical_value_uV / gain.
            # Here, assume gain means physical = (ADC_val - ADC_zero) * ADC_res / gain_factor.
            # Or simpler for dummy: ADC_val = physical_value * gain_for_dat
            # Let's assume gain 100 means 100 ADC units per mV.
            adc_ch1 = (signal_ch1 * 100).astype(np.int16) 
            adc_ch2 = (signal_ch2 * 100).astype(np.int16)
            
            # Interleave for format 212 (each sample from all channels, then next sample)
            dat_data = np.column_stack((adc_ch1, adc_ch2)) # This stacks them as columns
            # For wfdb tofile, it should be a flat array or structured if specific dtype.
            # For tofile with int16, it writes binary representation.
            # The wfdb library expects the .dat file to contain samples for each signal in succession for each sample instant.
            # So, if data is (N_samples, N_channels), this should be fine.
...
def main():
    # Attempt to create dummy WFDB files for basic testing if they don't exist.
    # This path is used as a default in the GUI.
